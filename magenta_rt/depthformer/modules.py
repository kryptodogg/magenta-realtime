...
def _to_depth_logit_mask(logit_mask: jnp.ndarray, num_levels: int):
    ...
    padded_mask = jnp.reshape(padded_mask, (padded_mask.shape[0], -1, num_levels, 1))
...
class DepthformerDecoderStack(nn.Module):
    ...
    def setup(self):
        self.temporal_decoder = PeriodicCallableTemporalDecoder(  # TemporalDecoderStack
...
class DepthformerDecoder(t5_architecture.Decoder):
    ...
    num_encoder_levels: int = 1
    gather_mode: str = "mean"
    shared_relative_position_bias_factory: Optional[Callable[[], nn.Module]] = None
    shared_relative_position_depth_bias_factory: Optional[Callable[[], nn.Module]] = (
        None
    )
...
class PeriodicCallableTemporalDecoder(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        call_counter = self.variable("cache", "call_counter", jnp.zeros, (), jnp.int32)
