...
def _make_decoder_layer(shared_relative_position_bias):
    ...
    return t5_architecture.DecoderLayer(
        ...
        encoder_decoder_attention=t5_test_utils.make_attention1(NUM_ATTN_HEADS, DTYPE),
...
def _make_depthformer_decoder(shared_token_embedder):
    ...
    return depthformer.DepthformerDecoder(
        ...
        shared_relative_position_depth_bias_factory=lambda: (
            t5_test_utils._make_relative_position_bias(  # pylint: disable=g-long-lambda
...
def _make_encoder_layer(shared_relative_position_bias):
    ...
    return t5_architecture.EncoderLayer(
        ...
        relative_position_bias_factory=(
            lambda: t5_test_utils._make_relative_position_bias(NUM_ATTN_HEADS, DTYPE)
        ),
        scanned=False,
        sow_intermediates=False,
    )
...
def _make_depthformer_encoder(shared_token_embedder):
    ...
    return depthformer.DepthformerEncoder(
        num_levels=2,
        num_layers=3,
        token_embedder_factory=(lambda: t5_test_utils.make_token_emb1(2_000, DTYPE)),
    ...
    def test_cached_mean_pooling(self):
        ...
        for idx in range(1, 3):
            _, new_vars = self.model.module.apply(
                ...
                jnp.expand_dims(self.input_batch["decoder_target_tokens"][:, idx], -1),
                enable_dropout=False,
                decode=True,
                max_decode_length=MAX_DECODE_LENGTH,
                mutable=["cache"],
