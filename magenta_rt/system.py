...
class MagentaRTBase(abc.ABC):
    ...
    def __init__(
        ...
        if config.style_rvq_codebook_size != self.style_model.config.rvq_codebook_size:
            raise ValueError(
...
class MockMagentaRT(MagentaRTBase):
    ...
    def generate_chunk(
        ...
        **kwargs,
        ...
        # Update state
        crossfade_samples = chunk_with_xfade[-self.config.crossfade_length_samples :]
...
class MagentaRTT5X(MagentaRTBase):
    ...
    def generate_chunk(
        ...
        **kwargs,
        ...
        assert codec_tokens_lm.shape == self.config.context_tokens_shape  # (250, 16)
        assert (
        ...
        assert style_tokens_lm.shape == (self.config.encoder_style_rvq_depth,)  # (6,)

        # Prepare encoder input
        batch_size, _, _ = self._device_params
        encoder_inputs_pos = np.concatenate(
            [
                codec_tokens_lm[:, : self.config.encoder_codec_rvq_depth].reshape(-1),
                style_tokens_lm,
            ],
            axis=0,
        )
        ...
        # Fade out last chunk
        chunk.samples[: self.config.crossfade_length_samples] += (
