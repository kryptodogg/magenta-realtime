...
def mask_inputs(
    ...
    def _apply_mask(ranges_to_mask):
        ...
        is_masked = tf.reduce_any(
            tf.stack(
                [tf.logical_and(r[0] <= arange, arange < r[1]) for r in ranges_to_mask]
            ),
            axis=0,
        )
...
def _load_style_tokens(
    ...
    tokens = (
        tf.reshape(tokens, (tokens.shape[0], -1)) if not keep_frame_dim else tokens
    )[0]
    return tokens
...
def preprocess_example(
    ...
    ex["targets"] = acoustic_tokens[
        end_i : end_i + num_gen_secs * int(codec_frame_rate) * decoder_codec_rvq_depth
    ]
...
def register_task(
    ...
    seqio_vocab = seqio.PassThroughVocabulary(magenta_rt_config.vocab_size_pretrained)
    ...
    seqio.TaskRegistry.add(
        ...
        preprocessors=[
            ...
            seqio.map_over_dataset(lambda ex: {**ex, "full_targets": ex["targets"]}),
            # pylint: enable=protected-access
            seqio.CacheDatasetPlaceholder(),
        ],
        output_features={
